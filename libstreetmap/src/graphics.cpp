...
/*Structure used to to define a text search box on the top left hand side of the main window
 *
 *
 *
 *
...
static void map_text_box();
static void build_text_box();
static void build_route_plan_window();
#ifdef X11
...
/* Map a text box onto the screen and set it up for input, etc.        
 * But in this case because we don't show the search box initially
 * So we iconfy the text_box window first
 */
static void map_text_box() {
    text_box.isOn = False;

#ifdef X11
        text_box.win = XCreateSimpleWindow(
        ...
        XSelectInput(x11_state.display, text_box.win, ExposureMask|KeyPressMask|KeyReleaseMask|KeymapStateMask);
//        text_box.draw = XftDrawCreate(
//            x11_state.display,
//            text_box.win,
//            x11_state.visual_info.visual,
//            x11_state.colormap_to_use
...
/* Used to setup text box and build it */
static void build_text_box() {
    ...
    std::string fontname("-*-helvetica-*-r-*-*-14-*-*-*-*-*-*-*");
    text_box.font = XLoadQueryFont (x11_state.display, fontname.c_str());
//    Info_box.font = XLoadQueryFont (x11_state.display, fontname.c_str());
//    /* If the font could not be loaded, revert to the "fixed" font. */
//    if (! text_box.font) {
//        fprintf (stderr, "unable to load font %s: using fixed\n", fontname.c_str());
//        text_box.font = XLoadQueryFont (x11_state.display, "fixed");
//    }
    XSetFont (x11_state.display, text_box.gc, text_box.font->fid);
    
    map_text_box();
}  

/* Show text input from keyboard on text_box */
void write_in_text_box(char keyValue) {
    //check if the input character is valid
    if(('A' <= keyValue && 'z'>= keyValue)|| keyValue == '&'
            || keyValue == ' '||'0'<=keyValue&&'9'>=keyValue||keyValue=='<'||keyValue=='>'||keyValue=='.'||keyValue=='\'')
        text_box.text.push_back(keyValue);
    else if (keyValue == '\b'&& text_box.text.length() > 0)
    ...
    int descent;
    XCharStruct overall;
    /* Centre the text in the left end of the box. */
    XTextExtents (text_box.font, text_box.text.c_str(), text_box.text.length(),
                    & direction, & ascent, & descent, & overall);
    x = 1;
    y = text_box.height / 2 + (ascent - descent) / 2;
    XClearWindow (x11_state.display, text_box.win);
    XDrawString (x11_state.display, text_box.win, text_box.gc,
                 x, y, text_box.text.c_str(), text_box.text.length());
}

/* Show or Hide Text box on the screen */
void show_text_box(bool turn_on){
    if(turn_on){
        text_box.text.clear();
        XClearWindow (x11_state.display, text_box.win);
        XMapWindow(x11_state.display, text_box.win);
        text_box.isOn = True;
    }else{
        //clear the contents in text box
        text_box.text.clear();
        XClearWindow (x11_state.display, text_box.win);
        XUnmapWindow(x11_state.display, text_box.win);
        //XIconifyWindow(x11_state.display,text_box.win,DefaultScreen(x11_state.display));
        text_box.isOn = False;
...
std::string read_text_box(){
    return text_box.text;
}

void show_menu(bool turn_on){
    if(turn_on)
...
/* this function is to build the left hand side route plan result*/
static void build_route_plan_window(){
    route_plan_window.Build(x11_state.display,x11_state.toplevel,0 ,0, MWIDTH * 3,trans_coord.top_height - T_AREA_HEIGHT ,
                x11_convert_to_xcolor(predef_colors[BLACK]), x11_convert_to_xcolor(t_color(52,105,175)));
}

void write_in_route_plane_window(std::string contents){
    route_plan_window.writeContents(contents);
}

void show_route_plane_window(bool turn_on){
    route_plan_window.displayWindow(turn_on);
}

void clear_route_plane_window(){
    route_plan_window.clearContents();
}
...
static void map_button(int bnum) {
    ...
    if (button_state.button[bnum].type != BUTTON_SEPARATOR) {
        ...
        button_state.button[bnum].win = XCreateSimpleWindow(
            ...
            x11_convert_to_xcolor(t_color(271,70,69)), 
            x11_convert_to_xcolor(t_color(271,70,69))
            );
...
build_default_menu(void) {
    ...
    x11_state.menu = XCreateSimpleWindow(
        ...
        x11_convert_to_xcolor(predef_colors[WHITE]),
        x11_convert_to_xcolor(t_color(31,37,61))
        //x11_convert_to_xcolor(predef_colors[BLACK]),
...
static void x11_init_graphics(const char *window_name) {
    ...
    text_box.gc = XCreateGC(x11_state.display, x11_state.toplevel, valuemask, &values);
...
//                printf("x: %d    y: %d\n", report.xmotion.x, report.xmotion.y);
                ...
                if (pan_state.panning_enabled)
                    ...
                    //pass in a very big number(can be treated as infinite) as out of window
                    act_on_mousemove(1.0e12,1.0e12);
                break;
            case KeyPress:
#ifdef VERBOSE 
...
static void x11_drawbut(int bnum) {
    ...
    if (ispressed) {
        XSetForeground(x11_state.display, x11_state.gc_menus, 
                x11_convert_to_xcolor(t_color(52,105,175)));
    } else {
        XSetForeground(x11_state.display, x11_state.gc_menus, 
                x11_convert_to_xcolor(t_color(52,105,175)));
    }
